...
function renderObs(d){$("sea").textContent=d?.seaTemp?String(d.seaTemp).replace(".",",")+"°":"—";$("air").textContent=d?.airTemp?String(d.airTemp).replace(".",",")+"°":"—";$("wind").textContent=d?.wind?String(d.wind).replace(".",",")+" m/s":"—";$("obs").textContent="Síðasta athugun Nauthólsvíkur: "+(d?.updated||"—")}
async function get(u){let r=await fetch(u+"?t="+Date.now(),{cache:"no-store"});if(!r.ok)throw Error(r.status);return r.json()}
